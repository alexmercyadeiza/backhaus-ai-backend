...
4. Apply migrations as a deliberate release step with a migration role. Run the service with a role limited to its application's tables. API startup never auto-migrates.
5. Adapt the systemd templates and install them only when the user authorizes deployment. Their 256/512 MiB memory settings are initial caps to evaluate, not measured capacity promises. The API and worker each have a small database pool; tune the combined budget alongside PostgreSQL and existing services.
